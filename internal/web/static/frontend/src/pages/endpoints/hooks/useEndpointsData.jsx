...
// - 多 Key 切换功能 (loadKeysOverview、switchKey)
...
//    - loadKeysOverview() - 加载所有端点的 Key 概览 (GET /api/v1/keys/overview)
//    - switchKey(endpointName, keyType, index) - 切换 Key (POST /api/v1/endpoints/{name}/keys/token or api-key)
// 4. 错误处理：完善的错误处理和用户反馈
...
const useEndpointsData = () => {
    ...
    const loadKeysOverview = useCallback(async () => {
        try {
            console.log('🔑 [端点React] 加载 Key 概览数据...');
            ...
            const responseData = await response.json();
            console.log('🔑 [端点React] Key 概览数据:', responseData);

            setKeysOverview(responseData);
            return responseData;

        } catch (error) {
            console.error('❌ [端点React] Key 概览加载失败:', error);
            return null;
        }
    }, []);
    ...
    const switchKey = useCallback(async (endpointName, keyType, index) => {
        try {
            console.log('🔄 [端点React] 切换 Key:', { endpointName, keyType, index });

            if (!endpointName) {
                throw new Error('端点名称不能为空');
            }

            if (keyType !== 'token' && keyType !== 'api_key') {
                throw new Error('无效的 Key 类型');
            }

            if (typeof index !== 'number' || index < 0) {
                throw new Error('无效的 Key 索引');
            }

            // 根据 keyType 选择 API 端点
            ...
            const result = await response.json();
            console.log('🔑 [端点React] Key 切换响应:', result);

            if (result.success) {
                console.log('✅ [端点React] Key 切换成功');
                ...
                return {
                    success: true,
                    message: result.message || 'Key 切换成功'
                };
            } else {
            ...
            console.error('❌ [端点React] Key 切换失败:', error);
            throw error; // 向上抛出错误，让 KeySelector 组件处理
        }
    }, [loadKeysOverview]);

    // ========== 批量操作方法 ==========

    // 批量健康检测所有端点（调用后端批量API）
    const performBatchHealthCheckAll = useCallback(async () => {
        try {
            console.log('🔍 [端点React] 开始批量健康检测所有端点');
            ...
            const result = await response.json();
            console.log('📡 [端点React] 批量健康检测响应:', result);

            if (result.success) {
                console.log('✅ [端点React] 批量健康检测完成:', {
                    total: result.total,
            ...
            console.error('❌ [端点React] 批量健康检测失败:', error);
            throw error;
        }
    }, [loadData]);
    ...
    const activateEndpointGroup = useCallback(async (endpointName, groupName) => {
        if (!groupName) {
            const error = new Error('端点未配置组信息');
            console.error('❌ [端点React] 激活组失败:', error.message);
            throw error;
        }

        try {
            console.log(`⚡ [端点React] 正在激活组 "${groupName}" (端点: ${endpointName})`);
            ...
            if (result.success) {
                console.log(`✅ [端点React] 组 "${groupName}" 激活成功`);
                ...
                return {
                    success: true,
                    message: result.message || `组 "${groupName}" 激活成功`,
                    activeGroup: result.active_group
                };
            } else {
                throw new Error(result.message || result.error || '组激活失败');
            }
        } catch (error) {
            console.error(`❌ [端点React] 激活组 "${groupName}" 失败:`, error);
            throw error;
        }
    }, [loadData]);
    ...
    return {
        ...
        performBatchHealthCheckAll,  // 调用后端批量API的方法
        activateEndpointGroup,       // 激活端点所在的组
