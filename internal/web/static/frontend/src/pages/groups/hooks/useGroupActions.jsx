...
const useGroupActions = (refreshGroups, showConfirmDialog) => {
    ...
    const activateGroup = async (groupName) => {
        try {
            ...
            if (window.Utils && window.Utils.showSuccess) {
                window.Utils.showSuccess(result.message || `组 ${groupName} 已激活`);
            }
            ...
            if (window.Utils && window.Utils.showError) {
                window.Utils.showError('激活组失败: ' + error.message);
            }
            return false;
        }
    };
    ...
    const pauseGroup = async (groupName) => {
        try {
            ...
            const result = await response.json();
            if (window.Utils && window.Utils.showSuccess) {
                window.Utils.showSuccess(result.message || `组 ${groupName} 已暂停`);
            }
            ...
            if (window.Utils && window.Utils.showError) {
                window.Utils.showError('暂停组失败: ' + error.message);
            }
            return false;
        }
    };
    ...
    const resumeGroup = async (groupName) => {
        try {
            ...
            const result = await response.json();
            if (window.Utils && window.Utils.showSuccess) {
                window.Utils.showSuccess(result.message || `组 ${groupName} 已恢复`);
            }
            ...
            if (window.Utils && window.Utils.showError) {
                window.Utils.showError('恢复组失败: ' + error.message);
            }
            return false;
        }
    };
    ...
    const forceActivateGroup = async (groupName) => {
        if (!showConfirmDialog) {
            if (window.Utils && window.Utils.showError) {
                window.Utils.showError('确认对话框不可用');
            }
            return false;
        }
        ...
        if (confirmed) {
            try {
                ...
                const result = await response.json();
                if (window.Utils && window.Utils.showWarning) {
                    window.Utils.showWarning(result.message || `组 ${groupName} 已应急激活`);
                }
                ...
                if (window.Utils && window.Utils.showError) {
                    window.Utils.showError('应急激活失败: ' + error.message);
                }
                return false;
            }
        }
