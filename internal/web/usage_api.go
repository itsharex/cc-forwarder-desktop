...
func (ua *UsageAPI) HandleUsageRequests(w http.ResponseWriter, r *http.Request) {
	...
	// Query request details (双源查询: 热池 + 数据库)
	details, total, err := ua.tracker.QueryRequestDetailsWithHotPool(ctx, opts)
	if err != nil {
...
func (ua *UsageAPI) HandleUsageStats(w http.ResponseWriter, r *http.Request) {
	...
	// Get filtered request details (双源查询: 热池 + 数据库)
	requests, _, err := ua.tracker.QueryRequestDetailsWithHotPool(ctx, opts)
	if err != nil {
