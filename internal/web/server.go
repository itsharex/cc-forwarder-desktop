...
// Embed 静态资源到二进制文件中
// 只嵌入构建产物 dist/ 和 favicon.ico，不包含 frontend/ 源代码
//go:embed static/dist static/favicon.ico
...
func (ws *WebServer) setupRoutes() {
	...
	if err != nil {
		ws.logger.Error("❌ 无法加载嵌入的静态文件", "error", err)
		panic(err)
	}

	// 服务 dist/assets 目录（所有 JS/CSS 资源）
	assetsFS, err := fs.Sub(distFS, "assets")
	if err != nil {
		ws.logger.Error("❌ 无法加载嵌入的 assets 目录", "error", err)
		panic(err)
	}
	ws.engine.StaticFS("/assets", http.FS(assetsFS))

	// 服务 favicon（从静态文件系统根目录）
	ws.engine.GET("/favicon.ico", func(c *gin.Context) {
	...
	// 服务 vite.svg（从 dist 目录）
	ws.engine.GET("/vite.svg", func(c *gin.Context) {
		data, err := staticFS.ReadFile("static/dist/vite.svg")
		if err != nil {
			c.Status(http.StatusNotFound)
			return
		}
	...
	api := ws.engine.Group("/api/v1")
	{
		...
		api.POST("/endpoints/health-check-all", ws.handleBatchHealthCheckAll)

		// Key 管理 API (多 API Key 切换功能)
		api.GET("/endpoints/:name/keys", ws.handleEndpointKeys)
		api.POST("/endpoints/:name/keys/token", ws.handleSwitchToken)
		api.POST("/endpoints/:name/keys/api-key", ws.handleSwitchApiKey)
		api.GET("/keys/overview", ws.handleAllEndpointKeys)
